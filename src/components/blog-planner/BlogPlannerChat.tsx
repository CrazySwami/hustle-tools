...
import { CopyIcon, RotateCcwIcon, GlobeIcon, SendIcon, Calendar } from 'lucide-react';
import { useState } from 'react';
import { cn } from '@/lib/utils';

interface BlogPlannerChatProps {
  messages: any[];
  isLoading: boolean;
  status?: string;
...
export function BlogPlannerChat({
  messages,
  isLoading,
  status,
  onSendMessage,
    ...
    <div style={{
                      ...
                      message.parts.filter(part => part != null).map((part: any, i: number) => {
                        ...
                        switch (part.type) {
                          ...
                          case 'tool-writeBlogPost': {
                            ...
                            // Extract tool name from part type (e.g., 'tool-planBlogTopics' → 'planBlogTopics')
                          ...
                          case 'tool-result': {
                            ...
                            // Use custom renderer for blog planner tools
                            if (toolName === 'planBlogTopics' || toolName === 'writeBlogPost') {
                              console.log('📊 Rendering blog planner widget for:', toolName);
                              return (
                                <ToolResultRenderer
                                  key={i}
                                  toolResult={{
      ...
      <PromptInput onSubmit={handleSubmit} style={{ flexShrink: 0, marginTop: '16px' }}>
        <PromptInputTextarea
          ...
          value={input}
          placeholder="Ask me to generate a blog content calendar or write a blog post..."
        />
        <PromptInputToolbar>
          <PromptInputTools>
            <PromptInputButton
