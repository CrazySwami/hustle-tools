...
// Model pricing per 1M tokens (Updated 2025 - Production Pricing)
export const MODEL_PRICING = {
  ...
  // ═══════════════════════════════════════════════════════════════
  // 🌀 Morph Models (via Vercel AI Gateway)
  // ═══════════════════════════════════════════════════════════════
  'morph/v3-fast': {
    input: 0.80,
    output: 1.20,
...
interface UsageTrackingStore {
  ...
  usageRecords: UsageRecord[]; // NEW: Detailed history
...
export const useUsageTracking = create<UsageTrackingStore>()(
  persist(
    (set, get) => ({
      ...
      totalCalls: 0,
      modelUsage: {},
      usageRecords: [], // NEW: Initialize empty records
      totalTokens: {
        input: 0,
        output: 0,
        cacheCreation: 0,
  ...
  recordUsage: (model: string, usage: TokenUsage) => {
    ...
    set({
      ...
      usageRecords: [...state.usageRecords, record], // Add to history
      totalTokens: {
    ...
    console.log('[useUsageTracking] Updated state:', {
      totalCalls: state.totalCalls + 1,
      totalCost: state.totalCost + callCost,
      recordsCount: state.usageRecords.length + 1,
    });
  },
  ...
  getTotalTokens: () => {
    const state = get();
    return state.totalTokens.input + state.totalTokens.output;
  },

  clearHistory: () => {
    console.log('[useUsageTracking] Clearing history');
    set({
      sessionStart: new Date(),
      totalCalls: 0,
      modelUsage: {},
  ...
  exportToCSV: () => {
    const state = get();
    const headers = ['Timestamp', 'Model', 'Input Tokens', 'Output Tokens', 'Cache Read Tokens', 'Cost'];
    const rows = state.usageRecords.map(r => [
...
}),
    {
      ...
      version: 2, // Incremented to force migration - added usageRecords array
    }
  )
);
